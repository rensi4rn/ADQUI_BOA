...
BEGIN
			...
			v_codigo=convert(NULL::varchar, 'LATIN1', 'UTF8');
			v_cod_prg=convert(p_cod_prg::varchar, 'LATIN1', 'UTF8');
			v_cod_pry=convert(p_cod_proy::varchar, 'LATIN1', 'UTF8');
			v_descripcion=convert(NULL::varchar, 'LATIN1', 'UTF8');
            if p_estado_pres=1 then            
				v_estado=convert('Formulación'::varchar, 'LATIN1', 'UTF8');
			elsif p_estado_pres=2 then
            	v_estado=convert('Verificación Previa'::varchar, 'LATIN1', 'UTF8');
            elsif p_estado_pres=3 then
            	v_estado=convert('Revisión Final'::varchar, 'LATIN1', 'UTF8');
            elsif p_estado_pres=4 then
            	v_estado=convert('Anteproyecto'::varchar, 'LATIN1', 'UTF8');
            elsif p_estado_pres=5 then
            	v_estado=convert('Aprobado'::varchar, 'LATIN1', 'UTF8');
            end if;
			...
			v_id_fuente_financiammiento=p_id_fuente_financiamiento::int4;
			--v_id_gestion=NULL::int4;
			...
			v_nombre_agrupador=convert(p_nombre_agrupador::varchar, 'LATIN1', 'UTF8');
